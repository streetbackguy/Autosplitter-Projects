...
init
{
    ...
    switch (MD5Hash)
        {
            ...
            case "769611513B0F5C3FE1FE7BEC479B1B60":
                version = "Steam 1.0.22";
                break;

            case "422088019C0D0A95673B89169272FC44":
                version = "Steam 1.0.44";
                break;
    ...
    // GWorld.StreamingLevels[0].Object.FName
    vars.Helper["SegmentName"] = vars.Helper.ReadString(gWorld, 0x2C0, 0x28, 0x0);

    // GEngine.GameInstance.LocalPlayer[0].PlayerController.BP_OurPlayer.ItemComponent.ItemHeldByPlayer
    vars.Helper["ItemHeld"] = vars.Helper.Make<ulong>(gEngine, 0x11F8, 0x38, 0x0, 0x30, 0x860, 0x9D0, 0xA0, 0x18);

    // GEngine.GameInstance.LocalPlayer[0].PlayerController.CollectibleWidget
    vars.Helper["Collectibles"] = vars.Helper.Make<ulong>(gEngine, 0x11F8, 0x38, 0x0, 0x30, 0x890, 0x18);
