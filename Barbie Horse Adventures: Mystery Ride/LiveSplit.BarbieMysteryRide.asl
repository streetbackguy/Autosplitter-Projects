...
startup
{
    ...
    settings.Add("BMR", true, "Barbie Horse Adventures: Mystery Ride");
        ...
        settings.Add("Canyon", true, "Canyon Race", "BMR");
            settings.Add("desert2", true, "Canyon Race Split after exiting Fruit Puzzle", "Canyon");
            settings.Add("desert", false, "Alternate Split on entering the Desert area after Canyon Race", "Canyon");
...
split
{
    ...
    if (current.AreaID == 1413298257 && old.Minigame == 1 && !vars.Splits.Contains("desert2"))
    {
        vars.Splits.Add("desert2");
        return settings["desert2"];
    }

    if (current.Minigame == 0 && old.Minigame == 1 && current.AreaID != 1413298257)
    {
